...
export function PresetCards({ selected, onSelect }: PresetCardsProps) {
  ...
  return (
    <div className={`grid gap-2 md:gap-4 w-full ${totalItems === 1
        ? 'grid-cols-1'
      ...
      {PRESETS.map((p) => {
        ...
        return (
          <button
            ...
            className={`group text-left p-3.5 md:p-4 rounded-xl md:rounded-2xl border-2 transition-all duration-300 select-none cursor-pointer flex flex-col justify-between min-h-[130px] md:min-h-[150px] relative overflow-hidden active:scale-[0.98] ${isSelected
          ...
          >
            ...
            <div className="relative z-10 w-full space-y-1.5 md:space-y-2">
              ...
              <div className='flex items-center gap-2.5'>
                <div
                  className={`p-2 md:p-2.5 rounded-xl md:rounded-xl transition-all duration-300 shrink-0 ${isSelected
                ...
                <span
                  className={`text-sm md:text-base font-extrabold truncate transition-colors duration-300 ${isSelected
            ...
            <div className='relative z-10 w-full flex items-center justify-between mt-2.5 pt-2 border-t border-green-teal-10/15 dark:border-neutral-80/10'>
              <span
                className={`text-[9px] md:text-[10px] font-bold tracking-wider uppercase px-2.5 py-1 md:px-3 md:py-1 rounded-lg transition-all duration-300 border ${isSelected
              ...
              {/* Radio Indicator */}
              <div
                className={`w-4 h-4 md:w-5 md:h-5 rounded-full border-2 transition-all duration-300 flex items-center justify-center ${isSelected
                    ? 'border-green-teal'
                    : 'border-green-teal-10/60 dark:border-neutral-80/20 group-hover:border-green-teal-30'
                  }`}
              >
                <div
                  className={`w-2 h-2 md:w-2.5 md:h-2.5 rounded-full bg-green-teal transition-all duration-300 ${isSelected ? 'scale-100 opacity-100' : 'scale-0 opacity-0'
                    }`}
                />
              </div>
            </div>
