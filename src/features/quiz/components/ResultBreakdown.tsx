...
function TypeRow({ label, total, correct }: { label: string; total: number; correct: number }) {
  ...
  return (
    <div className='flex items-center gap-4'>
      <span className='w-28 text-sm font-medium text-foreground shrink-0'>{label}</span>
      <div className='flex-1 h-3 bg-neutral-5 rounded-full overflow-hidden'>
      ...
      <span className='w-10 text-right text-sm font-semibold text-foreground shrink-0'>{pct}%</span>
    </div>
  )
}
...
export function ResultBreakdown({ typeStats }: ResultBreakdownProps) {
  ...
  return (
    <div className='space-y-3 p-6 bg-card rounded-2xl border border-border'>
      <h3 className='text-sm font-semibold text-foreground'>Kết quả theo chủ điểm</h3>
