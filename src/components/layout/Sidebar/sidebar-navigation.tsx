...
export default function SidebarNavigation({
  ...
  // prefetch all topics when module loads
  useEffect(() => {
    if (!currentPracticeContext?.module) return
    const moduleId = currentPracticeContext.module.id
    for (const topic of topics) {
      router.prefetch(`/practice/${moduleId}/${topic.slug}`)
    }
  }, [router, currentPracticeContext, topics])
                  ...
                  ${isOpen ? 'text-primary' : ''}`}
                      ...
                      {isExpanded && (
                        <span className='flex-1 text-sm whitespace-nowrap'>
                          {ConvertSlugToNameVietNam(topic.slug)}
                        </span>
                      )}
                      {isExpanded && (
                        <motion.span
                            ...
                            >
                              ...
                              >
                                <div className='flex items-center gap-1.5'>
                                  ...
                                  {FirstCharOfStringCapitalize(section.label)}
                                </div>
                              </button>
                            </motion.div>
                          ))}
